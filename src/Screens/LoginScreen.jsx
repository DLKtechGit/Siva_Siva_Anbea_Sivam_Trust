...
import { storeuser, storeUserData } from "../utils/auth";
...
const LoginScreen = ({ navigation }) => {
  ...
  const validation = () => {
    ...
    if (!logindata.email || logindata.email.trim() === "") {
      newerror.email = "Email field should not be empty!";
      isvalid = false;
    }
    // password validation
    if (!logindata.password || logindata.password.trim() === "") {
      newerror.password = "Password field should not be empty!";
      isvalid = false;
  ...
  const handlelogin = async () => {
    if (validation()) {
      ...
      try {
        ...
        const {id,token,name,email,image} = response.data.data;
