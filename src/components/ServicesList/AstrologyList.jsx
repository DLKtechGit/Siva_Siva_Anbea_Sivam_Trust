      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.modal_content}>
          ...
          <View style={styles.img_txt_container}>
            <Image source={{ uri: item.url }} style={styles.modal_img} />
            {/* name container */}
            <View style={{ display: "flex", flexDirection: "column" }}>
              <Text style={styles.modal_doc_name}>{item.name}</Text>

              <Text style={styles.modal_doc_speciality}>{item.category}</Text>
            </View>
          </View>
          <Text style={styles.modal_doc_desc}>{item.bio}</Text>
          <Text style={styles.modal_doc_fees}>Fees: {item.amount}</Text>
...
const AstrologyList = ({ id }) => {
  ...
  const fetchLists = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/get_service/${id}`, {
        headers: {
  ...
  const Bookbtn = (id, user_id) => {
    //setselectedbtn(i === selectedbtn ? null : i);
    //console.log(i)
    navigation.navigate("Astrologyform", { id, user_id });
  };
          ...
          return (
            ...
            <View key={i} style={styles.main_container}>
              ...
              <View style={styles.txt_container}>
                ...
                <Text style={styles.doc_speciality}>{item.category}</Text>
                <Text style={styles.doc_desc}>
                  {item.bio.length > 30 ? item.bio.slice(0, 60) : item.bio}
                  <Text>...</Text>
                </Text>
